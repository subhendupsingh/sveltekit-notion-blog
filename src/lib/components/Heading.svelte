<script lang="ts">
	...
	function getHeadingText(
		...
		if (!heading) return;
		let richText: TextRichTextItemResponse[] =
...
{#if heading}
	{#if heading.type == 'heading_1'}
		<h1 class="sk-blog-h1" id={slugIt(heading)}>
			{@html getHeadingText(heading)?.headingTextWithAnnotations}
		</h1>
	{:else if heading.type == 'heading_2'}
		<h2 class="sk-blog-h2" id={slugIt(heading)}>
			{@html getHeadingText(heading)?.headingTextWithAnnotations}
		</h2>
	{:else if heading.type == 'heading_3'}
		<h3 class="sk-blog-h3" id={slugIt(heading)}>
			{@html getHeadingText(heading)?.headingTextWithAnnotations}
		</h3>
	{/if}
{/if}
